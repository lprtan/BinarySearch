...
bool BFS(Dugum*, int);//Ilk genislik arama fonksiyonu bir agac koku ve aranacak deger parametrelerini alir
 ...
 void IkiliSiralamaAgaci::ekle(int eklenecek)
 {
         Dugum *tara, *yeni;
         tara = kok;
         bool eklendi = false;
 ...
 bool DFS(Dugum* kok, int aranacak)
 {
     Dugum* p;
 ...
 bool BFS(Dugum* kok, int aranacak) 
 {
     Dugum* p;
 ...
 void IkiliSiralamaAgaci::agacBosalt(Dugum* kok)
 {
     if (kok)
     {
         if (kok->sol != NULL)
